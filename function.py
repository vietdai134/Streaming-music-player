...
def display_random_songs():
    ...
    for song_info in list_songs:
        ...
        sub_button.configure(
            command=lambda url=song_url, title=video_title, thumbnail=thumbnail_url: add_to_album(url, title,
                                                                                                  thumbnail))
        ...
        song_button.configure(image=thumbnail_image, font=("Arial", 13, "bold"), anchor="w", text=video_title,
                              command=lambda url=song_url: add_to_playlist(url))
        ...
        if selected_stream:
            ...
            while value < AppStatus.volume_audio.duration_seconds - 1:
                ...
                time_label.configure(
                    text=format_duration(value) + "/" + format_duration(AppStatus.volume_audio.duration_seconds))
                time.sleep(1)
...
def display_list_album():
    ...
    if album_files:
        for album_file in album_files:
            ...
            album_button = customtkinter.CTkButton(album_window, font=("Arial", 13, "bold"), text=album_name,
                                                   command=lambda name=album_name: remove_album(name, album_window))
...
def display_songs_in_album(album_name):
    ...
    for song_info in album_songs:
        ...
        sub_button = customtkinter.CTkButton(button_frame, text=" - ", compound=tk.LEFT, width=30,
                                             command=lambda url=song_url, name=album_name: remove_song_album(name, url))
        sub_button.pack(side="left")
...
def get_all_album():
    for widget in inner_album_frame.winfo_children():
        if isinstance(widget, customtkinter.CTkButton):
            widget.destroy()
    remove_button = customtkinter.CTkButton(inner_album_frame, text="Delete Album", command=display_list_album,
                                            font=("Arial", 13, "bold"))
    ...
    if album_files:
        for album_file in album_files:
            album_name = album_file.replace("_album.txt", "")
            album_button = customtkinter.CTkButton(inner_album_frame, font=("Arial", 13, "bold"), text=album_name,
                                                   width=100,
                                                   command=lambda name=album_name: display_songs_in_album(name))
...
def add_to_album(song_url, video_title, thumbnail_url):
    ...
    center_window(album_window)
    create_button = customtkinter.CTkButton(album_window, font=("Arial", 13, "bold"), text="Create Album",
                                            command=lambda url=song_url, title=video_title,
                                                           thumbnail=thumbnail_url: create_album(url, title, thumbnail,
                                                                                                 album_window))
    ...
    if album_files:
        for album_file in album_files:
            album_name = album_file.replace("_album.txt", "")
            album_button = customtkinter.CTkButton(album_window, font=("Arial", 13, "bold"), text=album_name, width=100,
                                                   command=lambda name=album_name, url=song_url, title=video_title,
                                                                  thumbnail=thumbnail_url: (
                                                   write_album_to_txt(name, url, title, thumbnail),
                                                   destroy_album_window(album_window)))
...
def update_selected_songs_listbox(video_title, thumbnail_url, song_url):
    ...
    sub_button = customtkinter.CTkButton(button_frame, text=" - ", compound=tk.LEFT, width=30,
                                         command=lambda url=song_url: remove_song_playlist(url))
    sub_button.pack(side="left")

    song_button = customtkinter.CTkButton(button_frame, compound=tk.LEFT)
    song_button.pack(fill="x", padx=5, pady=5)
    song_button.image = thumbnail_image
    song_button.configure(image=thumbnail_image, anchor="w", text=video_title,
                          command=lambda url=song_url: play_selected_song_from_playlist(url),
                          font=("Arial", 13, "bold"))
...
def update_time_play():
    while not AppStatus.stop_threads:
        if AppStatus.current_value < AppStatus.volume_audio.duration_seconds - 1:
            ...
            time_label.configure(text=format_duration(AppStatus.current_value) + "/" + format_duration(
                AppStatus.volume_audio.duration_seconds))
            time.sleep(1)
        if AppStatus.stop_threads:
            break
...
def slide_play(value):
    ...
    if AppStatus.volume_value == 100:
        ...
        trimmed_audio_segment = trimmed_audio_segment.apply_gain(
            trimmed_audio_segment.dBFS * (100 - AppStatus.volume_value) / 100)
    AppStatus.audio = trimmed_audio_segment
    pause_resume()
    AppStatus.stop_threads = False
    ...
    try:
        ...
        if items:
            for item in items:
                ...
                song_button.configure(image=thumbnail_image, anchor="w", text=video_title, font=("Arial", 13, "bold"),
                                      command=lambda url=song_url: add_to_playlist(url))
                sub_button.configure(
                    command=lambda url=song_url, title=video_title, thumbnail=thumbnail_url: add_to_album(url, title,
                                                                                                          thumbnail))
